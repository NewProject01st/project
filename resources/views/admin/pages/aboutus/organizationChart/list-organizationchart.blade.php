     ...
     <div class="main-panel">
         <div class="content-wrapper">
             <div class="page-header">
                 <h3 class="page-title">
                     Organization Chart List <a href="{{ route('add-organizationchart') }}"
                         class="btn btn-sm btn-primary ml-3">+ Add</a>
                 </h3>
                 <nav aria-label="breadcrumb">
                     <ol class="breadcrumb">
                         <li class="breadcrumb-item"><a href="#">Master Management</a></li>
                         <li class="breadcrumb-item active" aria-current="page"> Organization Chart List</li>
                     </ol>
                 </nav>
             </div>
             <div class="row">
                                 ...
                                 <div class="col-12">
                                     <div class="table-responsive">
                                         <table id="order-listing" class="table table-bordered">
                                             ...
                                             <tbody>
                                                 @foreach ($organizationchart as $item)
                                                     <tr>
                                                         <td>{{ $loop->iteration }}</td>
                                                         <td>{{ $item->english_title }}</td>
                                                         <td>{{ $item->marathi_title }}</td>
                                                         <td> <img
                                                                 src="{{ asset('storage/images/aboutus/' . $item->english_image) }}" />
                                                         </td>
                                                         <td> <img
                                                                 src="{{ asset('storage/images/aboutus/' . $item->marathi_image) }}" />
                                                         </td>
                                                         <!-- <td>
         ...
         <form method="POST" action="{{ url('/delete-organizationchart') }}" id="deleteform">
             @csrf
             <input type="hidden" name="delete_id" id="delete_id" value="">
         </form>
         <form method="POST" action="{{ url('/show-organizationchart') }}" id="showform">
             @csrf
             <input type="hidden" name="show_id" id="show_id" value="">
         </form>
         <form method="POST" action="{{ url('/edit-organizationchart') }}" id="editform">
             @csrf
