 ...
 @section('content')
     <?php $data_permission = getPermissionForCRUDPresentOrNot('list-search-rescue-teams', session('permissions'));
     ?>
     <div class="main-panel">
         <div class="content-wrapper">
             <div class="page-header">
                 <h3 class="page-title">
                     Search Rescue Team List
                     {{-- @if (in_array('per_add', $data_permission))
                         <a href="{{ route('add-search-rescue-teams') }}" class="btn btn-sm btn-primary ml-3">+ Add</a>
                     @endif --}}
                                 ...
                                 <div class="col-12">
                                     <div class="table-responsive">
                                         <table id="order-listing" class="table table-bordered">
                                             ...
                                             <tbody>
                                                 @foreach ($searchrescueteams as $item)
                                                     <tr>
                                                         ...
                                                         <td> <img class="img-size"
                                                                 src="{{ Config::get('DocumentConstant.SEARCH_RESCUE_TEAM_VIEW') }}{{ $item->english_image }}" alt=" {{ strip_tags($item['english_title']) }} Image"/>
                                                         </td>
                                                         <td> <img class="img-size"
                                                                 src="{{ Config::get('DocumentConstant.SEARCH_RESCUE_TEAM_VIEW') }}{{ $item->marathi_image }}" alt=" {{ strip_tags($item['marathi_title']) }} प्रतिमा"/>
                                                         </td>
                                                         <!-- <td>
         ...
         <form method="POST" action="{{ url('/delete-search-rescue-teams') }}" id="deleteform">
             @csrf
             <input type="hidden" name="delete_id" id="delete_id" value="">
         </form>
         <form method="POST" action="{{ url('/show-search-rescue-teams') }}" id="showform">
             @csrf
