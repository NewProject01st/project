 ...
 @section('content')
     <?php $data_permission = getPermissionForCRUDPresentOrNot('list-state-emergency-operations-center', session('permissions'));
     ?>
     <div class="main-panel">
         <div class="content-wrapper">
             <div class="page-header">
                 <h3 class="page-title">
                     State Emergency Operations Center List
                     {{-- @if (in_array('per_add', $data_permission))
                         <a href="{{ route('add-state-emergency-operations-center') }}" class="btn btn-sm btn-primary ml-3">+
                             Add</a>
                     @endif --}}
                                                 ...
                                                 @foreach ($stateemergencyoperationscenter as $item)
                                                     <tr>
                                                         ...
                                                         <td> <img class="img-size"
                                                                 src="{{ Config::get('DocumentConstant.STATE_OPERATION_CENTER_VIEW') }}{{ $item->english_image }}" alt=" {{ strip_tags($item['english_title']) }} Image"/>
                                                         </td>
                                                         <td> <img class="img-size"
                                                                 src="{{ Config::get('DocumentConstant.STATE_OPERATION_CENTER_VIEW') }}{{ $item->marathi_image }}" alt=" {{ strip_tags($item['marathi_title']) }} प्रतिमा"/>
                                                         </td>
                                                         <!-- <td>
         ...
         <form method="GET" action="{{ url('/edit-state-emergency-operations-center') }}" id="editform">
             @csrf
