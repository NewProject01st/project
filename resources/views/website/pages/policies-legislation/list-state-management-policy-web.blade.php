        ...
        <section class="wf100 subheader">
            <div class="container">
                ...
                <ul>
                    ...
                    <li>
                        @if (session('language') == 'mar')
                            {{ Config::get('marathi.POLICIES_LEGISLATION.POLICIES_LEGISLATION_SUB_LINK3') }}
                        @else
                            {{ Config::get('english.POLICIES_LEGISLATION.POLICIES_LEGISLATION_SUB_LINK3') }}
                        @endif
                    </li>
                </ul>
            </div>
                                ...
                                @forelse ($data_output as $item)
                                    <tr class="">
                                        @if (session('language') == 'mar')
                                            ...
                                            <td><div class="d-flex justify-content-center"> <a href="{{ Config::get('DocumentConstant.STATE_DISASTER_POLICY_VIEW') }}{{ $item['marathi_pdf'] }}"
                                                    target="_blank"><img src="{{ asset('assets/images/pdf.png/') }}"
                                                        width="35px" height="35px"></a></div></td>
                                        @else
                                            <td><div class="d-flex justify-content-center">{{ $loop->iteration }}</div></td>
                                            <td><?php echo $item['english_title']; ?></td>
                                            <td><div class="d-flex justify-content-center"> <a href="{{ Config::get('DocumentConstant.STATE_DISASTER_POLICY_VIEW') }}{{ $item['english_pdf'] }}"
                                                    target="_blank"><img src="{{ asset('assets/images/pdf.png/') }}"
