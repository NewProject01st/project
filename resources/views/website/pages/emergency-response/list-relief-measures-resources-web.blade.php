    ...
    @section('content')
        ...
        <section class="wf100 subheader">
            <div class="container">
                ...
                <ul>
                    <li> <a href="{{ route('list-relief-measures-resources-web') }}">
                            @if (session('language') == 'mar')
                    ...
                    <li>
                        @if (session('language') == 'mar')
                            {{ Config::get('marathi.EMERGENCY_RESPONSES.EMERGENCY_RESPONSES_SUB_LINK5') }}
                        @else
                            {{ Config::get('english.EMERGENCY_RESPONSES.EMERGENCY_RESPONSES_SUB_LINK5') }}
                        @endif
                    </li>
                </ul>
            </div>
                            ...
                            @forelse ($data_output as $item)
                                <div class="deprt-txt">
                                    @if (session('language') == 'mar')
                                        ...
                                        <img src="{{ Config::get('DocumentConstant.RELIEF_MEASURES_RESOURCES_VIEW') }}{{ $item['marathi_image'] }}"
                                            class="d-block w-100 to_set_img_right relief_img" alt="{{ strip_tags($item['marathi_title']) }} छायाचित्र">
                                        <p style="text-align: justify;"> <?php echo $item['marathi_description']; ?></p>
                                    @else
                                        <h3><?php echo $item['english_title']; ?> </h3>
                                        <img src="{{ Config::get('DocumentConstant.RELIEF_MEASURES_RESOURCES_VIEW') }}{{ $item['english_image'] }}"
                                            class="d-block w-100 to_set_img_right relief_img" alt="{{ strip_tags($item['english_title']) }} Image">
