    ...
    @section('content')
        ...
        <section class="wf100 subheader">
            <div class="container">
                ...
                <ul>
                    <li> <a href="{{ route('list-evacuation-plans-web') }}">
                            @if (session('language') == 'mar')
                    ...
                    <li>
                        @if (session('language') == 'mar')
                            {{ Config::get('marathi.EMERGENCY_RESPONSES.EMERGENCY_RESPONSES_SUB_LINK6') }}
                        @else
                            {{ Config::get('english.EMERGENCY_RESPONSES.EMERGENCY_RESPONSES_SUB_LINK6') }}
                        @endif
                    </li>
                </ul>
            </div>
        ...
        <div class="main-content p60">
            ...
            <div class="department-details">
                <div class="container">
                    <div class="row">
                        <div class="col-md-9 evacuation_pln_img">
                            <!--Department Details Txt Start-->
                            @forelse ($data_output as $item)
                                <div class="deprt-txt">
                                    @if (session('language') == 'mar')
                                        <h3><?php echo $item['marathi_title']; ?> </h3>
                                        <img src="{{ Config::get('DocumentConstant.EVACUATION_PLAN_VIEW') }}{{ $item['marathi_image'] }}"
                                            class="d-block w-100" alt="{{ strip_tags($item['marathi_title']) }} छायाचित्र">
                                        <p style="text-align: justify;"> <?php echo $item['marathi_description']; ?></p>
                                    @else
                                        <h3><?php echo $item['english_title']; ?> </h3>
                                        <img src="{{ Config::get('DocumentConstant.EVACUATION_PLAN_VIEW') }}{{ $item['english_image'] }}"
                            ...
                            @empty
                                <h4>
                                    @if (session('language') == 'mar')
                                        {{ Config::get('marathi.EMERGENCY_RESPONSES.NO_DATA_FOUND_EVACUATION_PLANS') }}
                                    @else
                                        {{ Config::get('english.EMERGENCY_RESPONSES.NO_DATA_FOUND_EVACUATION_PLANS') }}
                                    @endif
                                </h4>
                            @endforelse
