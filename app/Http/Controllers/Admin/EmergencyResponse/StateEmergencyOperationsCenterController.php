...
use App\Http\Services\Admin\EmergencyResponse\StateEmergencyOperationsCenterServices;
use Validator;
use Config;
class StateEmergencyOperationsCenterController extends Controller
{
    ...
    $rules = [
        ...
        'english_image' => 'required|image|mimes:jpeg,png,jpg|max:'.Config::get("AllFileValidation.STATE_EMERGENCY_OPERATIONS_CENTER_IMAGE_MAX_SIZE").'|dimensions:min_width=1500,min_height=500,max_width=2000,max_height=1000|min:'.Config::get("AllFileValidation.STATE_EMERGENCY_OPERATIONS_CENTER_IMAGE_MIN_SIZE").'',
        'marathi_image' => 'required|image|mimes:jpeg,png,jpg|max:'.Config::get("AllFileValidation.STATE_EMERGENCY_OPERATIONS_CENTER_IMAGE_MAX_SIZE").'|dimensions:min_width=1500,min_height=500,max_width=2000,max_height=1000|min:'.Config::get("AllFileValidation.STATE_EMERGENCY_OPERATIONS_CENTER_IMAGE_MIN_SIZE").'',    
     ];
    $messages = [   
    ...
    public function update(Request $request)
{
    $rules = [
     ...
     if($request->has('english_image')) {
        $rules['english_image'] = 'required|image|mimes:jpeg,png,jpg|max:'.Config::get("AllFileValidation.STATE_EMERGENCY_OPERATIONS_CENTER_IMAGE_MAX_SIZE").'|dimensions:min_width=1500,min_height=500,max_width=2000,max_height=1000|min:'.Config::get("AllFileValidation.STATE_EMERGENCY_OPERATIONS_CENTER_IMAGE_MIN_SIZE");
    }
    if($request->has('marathi_image')) {
        $rules['marathi_image'] = 'required|image|mimes:jpeg,png,jpg|max:'.Config::get("AllFileValidation.STATE_EMERGENCY_OPERATIONS_CENTER_IMAGE_MAX_SIZE").'|dimensions:min_width=1500,min_height=500,max_width=2000,max_height=1000|min:'.Config::get("AllFileValidation.STATE_EMERGENCY_OPERATIONS_CENTER_IMAGE_MIN_SIZE");
    }
    $messages = [   
        'english_title.required'=>'Please enter title.',
        'english_title.regex' => 'Please  enter text only.',
