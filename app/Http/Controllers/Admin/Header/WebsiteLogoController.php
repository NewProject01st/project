...
use App\Http\Services\Admin\Header\WebsiteLogoServices;
use Validator;
use Config;
class WebsiteLogoController extends Controller
{
    ...
    public function update(Request $request){
        ...
        if($request->has('logo')) {
            $rules['logo'] = 'required|image|mimes:png|max:'.Config::get("AllFileValidation.WEBSITE_LOGO_MAX_SIZE").'|dimensions:min_width=200,min_height=100,max_width=800,max_height=400|min:'.Config::get("AllFileValidation.WEBSITE_LOGO_MIN_SIZE");
        }
    $messages = [   
