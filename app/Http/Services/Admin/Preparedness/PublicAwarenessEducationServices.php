...
use App\Http\Repository\Admin\Preparedness\PublicAwarenessEducationRepository;
           ...
           $return_data = $this->repo->updateAll($request);
            ...
            if ($request->hasFile('english_image')) {
                if ($return_data['english_image']) {
                    if (file_exists_s3(Config::get('DocumentConstant.PUBLIC_AWARENESS_EDUCATION_DELETE') . $return_data['english_image'])) {
                        removeImage(Config::get('DocumentConstant.PUBLIC_AWARENESS_EDUCATION_DELETE') . $return_data['english_image']);
                    }
                }
            ...
            if ($request->hasFile('marathi_image')) {
                if ($return_data['marathi_image']) {
                    if (file_exists_s3(Config::get('DocumentConstant.PUBLIC_AWARENESS_EDUCATION_DELETE') . $return_data['marathi_image'])) {
                        removeImage(Config::get('DocumentConstant.PUBLIC_AWARENESS_EDUCATION_DELETE') . $return_data['marathi_image']);
                    }
